module.exports.config = {
  ...
  credits: "Mirai Team",
  hasPermssion: 0,
  description: "Answer questions (in English)",
...
module.exports.handleReaction = ({ api, event, handleReaction }) => {
  ...
  if (response == handleReaction.answer) api.sendMessage("galing mo naman idol pwede ka na ilaban sa quiz bee", event.threadID);
  else api.sendMessage("wrong, nice try bawi na lng sa next quiz", event.threadID);
...
module.exports.run = async ({ api, event, args }) => {
  ...
  if (!fetch.data) return api.sendMessage("Unable to find a question due to server busy", event.threadID, event.messageID);
  return api.sendMessage(`Here's a question for you:\n\n        ${decodeURIComponent(fetch.data.results[0].question)}\n\n   👍🏻: True       👎🏻: False`, event.threadID, async (err, info) => {
    ...
    if (data.answerYet !== 1) {
      api.sendMessage(`Times up! The correct answer is ${fetch.data.results[0].correct_answer}`, event.threadID, info.messageID);
